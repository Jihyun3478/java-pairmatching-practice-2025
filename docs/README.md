## 1. 프로젝트 개요
미션을 함께할 페어를 관리해주는 프로젝트입니다.<br>
과정, 레벨, 미션을 입력해주시면, 페어 매칭 및 매칭 결과를 조회하실 수 있습니다!
...
- [x] 기능을 입력받는다.
  - [x] [예외상황] '1' 또는 '2' 또는 '3' 또는 'Q'가 아닌 경우, "[ERROR] 존재하지 않는 기능입니다. 다시 입력해주세요."를 출력한다.


- [x] 과정, 레벨, 미션을 입력받는다.
  - [x] [예외상황] 입력값이 비어있을 경우, "[ERROR] 입력값이 빈 값입니다. 다시 입력해주세요."를 출력한다.
  - [x] [예외상황] 콤마(,)를 기준으로 입력되지 않은 경우, "[ERROR] 잘못된 형식입니다. 다시 입력해주세요."를 출력한다.
  - [x] [예외상황] 존재하지 않는 과정인 경우, "[ERROR] 존재하지 않는 과정입니다. 다시 입력해주세요."를 출력한다.
  - [x] [예외상황] 존재하지 않는 레벨인 경우, "[ERROR] 존재하지 않는 레벨입니다. 다시 입력해주세요."를 출력한다.
  - [x] [예외상황] 존재하지 않는 미션인 경우, "[ERROR] 존재하지 않는 미션입니다. 다시 입력해주세요."를 출력한다.


- [ ] 매칭 정보가 존재할 경우, 재매칭 여부를 입력받는다.
    - [ ] [예외상황] '네' 또는 '아니오'가 아닌 경우, "[ERROR] 잘못된 형식입니다. 다시 입력해주세요."를 출력한다.
...
### 같은 레벨에서 같은 페어 재매칭하지 않는 기능
- [x] 같은 레벨에서 이미 페어를 맺은 크루와는 다시 페어로 매칭될 수 없다.
  - [x] 다시 페어로 매칭 된다면 크루 목록의 순서를 다시 랜덤으로 섞어서 매칭을 시도한다.
...
- [x] 매칭 이력이 없으면 "[ERROR] 매칭 이력이 없습니다."라고 안내한다.
...
- 기능 목록 작성: 35분
- MVP 완성: 5시간 8분 (기능 목록 작성 + 4시간 33분)
- 리팩토링:
- 총 구현 시간:
- 추가 구현 시간: 
