## 1. 프로젝트 개요
우테코의 미션은 페어 프로그래밍으로 진행됩니다.
해당 프로젝트는 미션을 함께할 페어를 관리해주는 애플리케이션 입니다.

---

## 2. 기능 목록
### 입력 처리
- [x] 크루 정보를 제공된 파일로부터 입력받는다.
  - [x] [예외 상황] 파일 이름이 존재하지 않거나 잘못된 형식의 파일이 입력된 경우


- [x] 기능을 입력을 받는다.
    - [x] [예외 상황] 입력값이 비어있는 경우
    - [x] [예외 상황] 입력값이 숫자가 아닌 경우


- [x] 과정, 레벨, 미션을 입력받는다.
  - [x] [예외 상황] 입력값이 비어있는 경우
  - [x] [예외 상황] 입력값이 쉼표(, )를 기준으로 구분되어 입력되지 않은 경우
  - [x] [예외 상황] 입력값 사이에 공백이 입력되지 않은 경우
  - [x] [예외 상황] 당첨 번호에서 쉼표(,)가 연속으로 입력된 경우
  - [x] [예외 상황] 당첨 번호에서 쉼표(,)로 시작하거나 끝나는 경우
  - [x] [예외 상황] 존재하지 않는 과정을 입력한 경우
  - [x] [예외 상황] 존재하지 않는 레벨을 입력한 경우
  - [x] [예외 상황] 존재하지 않는 미션을 입력한 경우


- [ ] 페어 재매칭 여부를 입력받는다.
  - [ ] [예외 상황] 입력값이 비어있는 경우
  - [ ] [예외 상황] 입력값이 '네' 또는 '아니오' 중에 입력되지 않은 경우
...
### 출력 처리
- [x] 사용자가 선택할 수 있도록 기능의 종류를 출력한다.


- [x] 사용자가 선택할 수 있도록 과정, 레벨, 미션을 출력한다.
...
- 입력값이 비어있는 경우
  - 판단: 잘못된 입력으로 간주하고, 다시 입력받는다.
- 콤마(,)를 기준으로 구분되어 입력되지 않은 경우
  - 판단: 잘못된 입력으로 간주하고, 다시 입력받는다.
- 공백을 기준으로 구분되어 입력되지 않은 경우
  - 판단: 잘못된 입력으로 간주하고, 다시 입력받는다.
- 존재하지 않는 과정, 레벨, 미션을 입력한 경우
  - 판단: 잘못된 입력으로 간주하고, 다시 입력받는다.
- 쉼표(,)가 연속으로 입력된 경우
  - 판단: 잘못된 입력으로 간주하고, 다시 입력받는다.
- 쉼표(,)로 시작하거나 끝나는 경우
...
### 재매칭 여부 입력 관련
- 입력값 사이에 공백이 입력되지 않은 경우
  - 판단: 잘못된 입력으로 간주하고, 다시 입력받는다.
- 입력값이 '네' 또는 '아니오' 중에 입력되지 않은 경우
  - 판단: 잘못된 입력으로 간주하고, 다시 입력받는다.
...
- [x] 파일 입출력 방법은 `자바 파일 읽기`나 `자바 파일 입출력`과 같은 키워드로 구글링해서 찾을 수 있다.
...
## 6. 구현 시간
- 기능 목록 작성: 38분
- 5시간 종료: 페어 매칭 기능 구현까지 완료 (기능 목록 작성 + 4시간)
- TDD 종료: 
- MVP 완성: 
- 리팩토링: 
- 총 구현 시간:
